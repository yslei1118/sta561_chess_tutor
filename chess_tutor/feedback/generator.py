...
class FeedbackGenerator:
    ...
    def __init__(self, complexity_estimator=None, stockfish_evaluator=None):
    ...
    def analyze_position_for_feedback(self, board: chess.Board,
        ...
        # Blunder probability estimated from hanging pieces (position heuristic).
        # ML-based blunder detection was attempted (§5) but achieved AUC≈0.51,
        # so we use the direct board signal instead.
        from ..data.extract_features import extract_board_features, count_hanging_pieces
        import numpy as np
        feat = extract_board_features(board)
        hanging = int(feat[29])
        king_safety = float(feat[14])
        result["blunder_prob"] = min(0.1 + 0.25 * hanging + 0.1 * (king_safety < 2), 1.0)
