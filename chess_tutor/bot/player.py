...
class ChessTutorBot:
    ...
    def _get_human_probs(self, board, legal_moves, target_elo):
        ...
        We compute a feature-based score per move and convert it to a
        distribution via softmax with ELO-dependent temperature:
          - Low ELO (1100): T≈2.0 -> flatter distribution (noisier play)
          - High ELO (1900): T≈0.4 -> sharper distribution (decisive play)

        Move-level feature weights also depend on skill so that beginners
        and experts genuinely evaluate the same move differently
        (beginners love captures/checks and miss threats; experts prioritize
        development, central control, king safety, and avoid hanging pieces).

        Note: we do not use the trained MovePredictor artifact in the live
        demo because that model is a candidate-ranking classifier expecting
        per-move (board + move) feature vectors via
        scripts/build_candidate_dataset.py. Wiring it into the interactive
        bot would require running the full candidate feature pipeline per
        legal move on every turn. The heuristic below is self-contained,
        fast, and produces visibly different behavior across ELO brackets.
        """
