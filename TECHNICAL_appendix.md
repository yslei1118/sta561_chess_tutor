...
10. [ELO-conditioned bot and A+ interactive entry points](#10-elo-conditioned-bot-and-a-interactive-entry-points)
...
**Operating system**: developed and tested on macOS 14 (darwin 25.3.0) and Windows 11.
...
**External binary (optional)**: Stockfish 15+ for ground-truth cp_loss labelling.
The absolute path is hard-coded in [scripts/label_real_blunders.py:23](scripts/label_real_blunders.py#L23) and must be edited for your machine:

```python
STOCKFISH_PATH = r"<path-to-stockfish-executable>"
```
...
All main-table numbers in §12 use the real Stockfish labels we pre-computed
and ship in `data/processed/real_cp_losses.npy`.
...
pytest tests/          # 13 test files, full suite should pass
```
...
|---|---|---:|---|
| `data/raw/lichess_2013_01.pgn` | [database.lichess.org/standard/lichess_db_standard_rated_2013-01.pgn.zst](https://database.lichess.org/standard/lichess_db_standard_rated_2013-01.pgn.zst) | ~1.2 GB uncompressed | primary training set |
| `data/raw/lichess_2014_01.pgn` | [database.lichess.org/standard/lichess_db_standard_rated_2014-01.pgn.zst](https://database.lichess.org/standard/lichess_db_standard_rated_2014-01.pgn.zst) | ~3.0 GB uncompressed | held-out / extra positions |

**Download command** (the `.zst` file must be decompressed to `.pgn`):

```bash
mkdir -p data/raw
curl -L -o data/raw/lichess_2013_01.pgn.zst \
    https://database.lichess.org/standard/lichess_db_standard_rated_2013-01.pgn.zst
zstd -d data/raw/lichess_2013_01.pgn.zst -o data/raw/lichess_2013_01.pgn
```
...
Script: [chess_tutor/data/parse_pgn.py](chess_tutor/data/parse_pgn.py), entry
point in [scripts/run_phase3_4_5.py](scripts/run_phase3_4_5.py).

```bash
python -c "
from chess_tutor.data.parse_pgn import parse_pgn_file
df = parse_pgn_file('data/raw/lichess_2013_01.pgn',
...
For our run: 15,000 positions per bracket × 5 brackets = 75,000 positions,
and ~30 legal moves/position ⇒ **~2.3 M rows total**, with a ≈3.3 % positive
rate.
...
Parallelism: 8 worker processes (set `N_WORKERS` in the script); wall time
≈ 30–90 minutes on an 8-core machine.
...
### 7.1 Linear Thompson Sampling (Agrawal & Goyal, ICML 2013)

Per-arm Bayesian linear regression with conjugate Gaussian posterior:

$$
B_a = \lambda I + \sum_t x_t x_t^{\top}, \qquad f_a = \sum_t r_t x_t,
$$
$$
\hat{\mu}_a = B_a^{-1} f_a, \qquad \Sigma_a = v^{2}\, B_a^{-1}.
$$

**Action selection**: for each arm, sample `θ̃_a ∼ N(μ̂_a, Σ_a)`,
pick `argmax_a θ̃_aᵀ x_t`.

**Hyperparameters** ([config.py:92-93](chess_tutor/config.py#L92-L93)):
- `v = 0.5` (exploration scale)
- `prior_variance = 0.5` (λ = 1 / 0.5 = 2)

**Numerical safety** ([bandit.py:35-37](chess_tutor/teaching/bandit.py#L35-L37)):
```python
cov = (cov + cov.T) / 2            # force symmetry
cov += 1e-6 * np.eye(d)            # ridge
```
Verified positive-definite for 1000 random updates in
[tests/test_bandit.py](tests/test_bandit.py) (`test_ts_stability_after_many_updates`).

### 7.2 LinUCB (Li et al., WWW 2010)

$$
A_a = I + \sum_t x_t x_t^{\top}, \quad b_a = \sum_t r_t x_t, \quad \theta_a = A_a^{-1} b_a
$$
$$
...
### 7.3 ε-Greedy baseline

`ε = 0.1`. With probability ε play a uniform random arm, else play the
arm with highest sample-mean reward.

### 7.4 Random baseline

Uniform random arm; still calls `update` so pull counts are tracked.

### 7.5 Rule-based baseline ([bandit.py:170](chess_tutor/teaching/bandit.py#L170))

```python
if context[5] > 0.5:  return 2  # BLUNDER_WARNING  (high blunder prob)
if context[4] > 0.7:  return 6  # SIMPLIFICATION    (very complex)
if context[4] > 0.5:  return 0  # TACTICAL_ALERT    (complex)
return 1                        # STRATEGIC_NUDGE   (default)
```

### 7.6 AlwaysTactical baseline

Degenerate policy that always selects arm 0. Used to bound the single-arm
ceiling and, by comparison, measure the value of using context at all.

### 7.7 Hyperparameter sweep
...
## 10. ELO-conditioned bot and A+ interactive entry points

### 10.1 `ChessTutorBot` ([chess_tutor/bot/player.py](chess_tutor/bot/player.py))

Two public methods used by the A+ interfaces:
...
### 10.2 A+ requirement 1 — FEN + target ELO → structured report
...
### 10.3 A+ requirement 2 — Interactive play
...
|---|---|---|
| [regret_curves.png](results/plots/regret_curves.png) | run_final_experiment.py | 5-policy cumulative regret ± 2 SEM |
| [arm_distribution.png](results/plots/arm_distribution.png) | run_final_experiment.py | arm selection frequency by policy |
| [elo_trajectories.png](results/plots/elo_trajectories.png) | run_final_experiment.py | 300-step ELO trajectories |
| [teaching_effectiveness.png](results/plots/teaching_effectiveness.png) | run_final_experiment.py | mean ELO gain ± std per policy |
| [ablation_table.png](results/plots/ablation_table.png) | run_final_experiment.py | ablation-table heatmap |
| [continuous_elo_interpolation.png](results/plots/continuous_elo_interpolation.png) | continuous_elo_visualization.py | C at ELO ∈ [900, 1900] |
| [cross_elo_heatmap.png](results/plots/cross_elo_heatmap.png) | run_phase3_4_5.py | Arch A train-bracket × test-bracket Top-1 |
| [bandwidth_cv.png](results/plots/bandwidth_cv.png) | run_phase3_4_5.py | LOO-CV curve for bandwidth selection |
| [feature_importance.png](results/plots/feature_importance.png) | run_phase3_4_5.py | RF Gini importance, top 20 |
| [roc_curve.png](results/plots/roc_curve.png) | run_phase3_4_5.py | blunder-detection ROC |
| [kernel_weights.png](results/plots/kernel_weights.png) | run_phase3_4_5.py | NW kernel weights vs target ELO |
| [*_phase4_small.png](results/plots/) | run_phase3_4_5.py | smaller-scale variants used in intermediate development |
...
13 pytest files under [tests/](tests/). Total runtime < 60 s on a laptop
(no GPU, no Stockfish required for the default suite):

```bash
python -m pytest tests/ -v
```

Coverage of behavioural-critical properties:

- **Features**: correctness of piece counts / material balance / king
  safety on tabulated reference positions
  ([tests/test_features.py](tests/test_features.py)).
- **Context**: 20-D shape, normalisation to ~[0, 1]
  ([tests/test_context.py](tests/test_context.py)).
- **Bandit numerical stability**: `Σ_a` remains positive-definite after
  1000 random updates; every policy returns a valid arm
  ([tests/test_bandit.py](tests/test_bandit.py)).
- **Kernel**: NW normalisation, monotonic re-weighting as bandwidth → ∞
  ([tests/test_kernel.py](tests/test_kernel.py)).
- **Student dynamics**: mastery stays in [0, 1]; forgetting strictly
  monotone in absence of feedback; Welch test triggers only after 30+
  samples ([tests/test_student.py](tests/test_student.py)).
- **Reward**: monotone in cp improvement; clipped to [0, 1]; robust
  to missing args ([tests/test_reward.py](tests/test_reward.py)).
- **Integration**: a 10-step end-to-end episode runs without errors
  ([tests/test_integration.py](tests/test_integration.py)).

---

## 15. Reproducibility checklist (step-by-step)

Starting from a bare clone of the repository:
...
# 2a. [OPTIONAL] re-run data pipeline from raw Lichess
#     skip if you want to use the shipped processed tensors
curl -L -o data/raw/lichess_2013_01.pgn.zst \
    https://database.lichess.org/standard/lichess_db_standard_rated_2013-01.pgn.zst
zstd -d data/raw/lichess_2013_01.pgn.zst -o data/raw/lichess_2013_01.pgn
python -c "from chess_tutor.data.parse_pgn import parse_pgn_file; \
           parse_pgn_file('data/raw/lichess_2013_01.pgn').to_parquet('data/processed/parsed_positions.parquet')"
python scripts/build_candidate_dataset.py
python scripts/label_real_blunders.py      # requires Stockfish on PATH

# 2b. [OR] use shipped tensors — just skip step 2a

# 3. Train the human-move imitation models (produces the two .pkl files)
python scripts/train_and_evaluate.py

# 4. Run the main experiment (produces all results/*.csv and most plots)
python scripts/run_final_experiment.py

# 5. Optional supplementary experiments
python scripts/bandit_hyperparam_sweep.py    # results/hyperparam_sweep.csv
python scripts/arch_c_retune.py              # results/arch_c_retune.csv
python scripts/continuous_elo_visualization.py  # results/continuous_elo_curves.csv + plot

# 6. Tests
python -m pytest tests/
```

Expected runtimes on an Apple M-series CPU:

| Step | Time |
|---|---:|
| Parse PGN | 10–20 min |
| Build candidate dataset | ~5 min |
| Stockfish labelling (depth 12, 8 workers) | 30–90 min |
| `train_and_evaluate.py` | 3–6 min |
| `run_final_experiment.py` | 15–30 min |
| `bandit_hyperparam_sweep.py` | ~15 min |
| `arch_c_retune.py` | 10–20 min |
| `pytest` | < 60 s |

**Interactive verification of A+ features**:

```bash
# A+ requirement 1
python -c "
from chess_tutor.interactive import evaluate_fen, format_report
print(format_report(evaluate_fen(
...
│   ├── bot/
│   │   ├── player.py              # ChessTutorBot (play_move, evaluate_position)
│   │   └── commentary.py          # tiered move-by-move commentary
│   ├── data/
│   │   ├── parse_pgn.py           # PGN → parquet
│   │   ├── dataset.py             # train/val/test splits
│   │   ├── extract_features.py    # 30-D board + 10-D move features
│   │   ├── stockfish_eval.py      # wrapper around python-chess engine
│   │   └── download.py            # Lichess / HF Hub downloaders
│   ├── evaluation/
│   │   ├── ablation.py            # arch × classifier ablation loop
│   │   └── metrics.py             # Top-K, regret, arm entropy, AUC
│   ├── feedback/
│   │   ├── taxonomy.py            # FeedbackType enum + concept map
│   │   ├── templates.py           # per-tier template store
│   │   └── generator.py           # FeedbackGenerator.generate(...)
│   ├── interactive/
│   │   ├── position_evaluator.py  # evaluate_fen / format_report (A+ #1)
│   │   └── game.py                # InteractiveGame + play_cli (A+ #2)
│   ├── models/
│   │   ├── candidate_predictor.py # Arch-A/B wrapper
│   │   ├── kernel_interpolation.py# Arch-C (Nadaraya–Watson)
│   │   ├── move_predictor.py      # legacy multiclass predictor
│   │   └── position_eval.py       # position assessment helpers
│   ├── simulation/
│   │   ├── student_simulator.py   # StudentSimulator + Population
│   │   └── runner.py              # run_episode / run_experiment / sanity
│   ├── student/
│   │   └── model.py               # StudentState + Welch trend
│   ├── teaching/
│   │   ├── bandit.py              # TS, LinUCB, ε-Greedy, rule, random
│   │   ├── context.py             # build_context (20-D)
│   │   └── reward.py              # compute_reward (pedagogical)
│   └── utils/
│       ├── helpers.py             # sigmoid / softmax / normalize_elo
│       └── visualization.py       # plotting utilities
├── scripts/
│   ├── build_candidate_dataset.py
│   ├── label_real_blunders.py
│   ├── train_and_evaluate.py
│   ├── arch_c_retune.py
│   ├── bandit_hyperparam_sweep.py
│   ├── continuous_elo_visualization.py
...
├── data/
│   ├── raw/                       # Lichess PGNs
│   ├── processed/                 # candidate_*.npy, real_cp_losses.npy, ...
│   └── demo_cache/                # shipped snapshot for quick demos
├── models/saved/
│   ├── models_a_candidate.pkl     # dict {1100, 1300, 1500, 1700, 1900: RF}
│   └── model_b_candidate.pkl      # single pooled RF
└── results/
    ├── bandit_comparison.csv
    ├── ablation_table.csv
    ├── arch_c_retune.csv
    ├── hyperparam_sweep.csv
    ├── continuous_elo_curves.csv
    └── plots/                     # 15 PNGs
```
...
*All code, data tensors, trained models, CSVs, plots, and tests
referenced above are included in the repository under
[sta561_project/](./). The same numbers reported in the main writeup
can be regenerated by running the commands in §15.*
